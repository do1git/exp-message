...
## 테스트 결과 (`04-test/01-login-performance`)
...
| LOCKED (잠금) | 16개 | 19개 |
| 평균 응답 시간 | 460ms | 320ms |
| P95 응답 시간 | 491ms | 344ms |
...
- USER_001 4번 vs 1번
  ...
  - 락이 있는 경우 현재 waiting이 없기 때문에 동시 요청은 첫번째 제외 모두 block
...
- 락 실패 시 DB 조회를 생략하도록 만들어 평균 응답 시간 460ms → 320ms로 개선(~30%)
...
- 현재 IP 추출이 `X-Forwarded-For` 우선이라 유저가 헤더를 조작해 IP를 임의로 바꿀 수 있어(락/카운팅 우회 가능) **프록시에서 XFF 덮어쓰기/정제 또는 trusted proxy 설정 패치 필요**
