...
class WebSocketClient {
   ...
   */
  connect(onMessage, ipAddress) {
    return new Promise((resolve, reject) => {
      const wsURL = `${this.baseURL}/ws?access_token=${this.accessToken}`;
      const options = {
        transportOptions: {
          websocket: { headers: { 'X-Forwarded-For': ipAddress }},
          xhr_streaming: { headers: { 'X-Forwarded-For': ipAddress}},
          xhr: { headers: { 'X-Forwarded-For': ipAddress}}
        },
        infoReceiver: {  headers: { 'X-Forwarded-For': ipAddress } }
      };
