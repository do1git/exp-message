package site.rahoon.message.monolithic.loginfailure.controller
...
import site.rahoon.message.monolithic.authtoken.controller.AuthRequest
import site.rahoon.message.monolithic.common.test.ConcurrentTestUtils
...
class LoginFailureControllerIT(
    ...
    fun `race condition 테스트 - 동시 요청 시 실패 횟수와 잠금 동작 검증`() {
        ...
        val responseBodies =
            ConcurrentTestUtils.executeConcurrent(threadCount) {
                try {
                    restTemplate
                        .exchange(
                            "${authBaseUrl()}/login",
        ...
        if (actualExpireTime != null) {
            val isInExpectedRange =
                !actualExpireTime.isBefore(expectedMinExpireTime) &&
    ...
    private fun extractErrorCode(responseBody: String?): String? {
        ...
        return try {
            ...
            if (responseMap["success"] == true) {
                ...
                (responseMap["error"] as? Map<*, *>)?.get("code") as? String ?: "UNKNOWN"
            }
        } catch (e: Exception) {
            "UNKNOWN"
        }
