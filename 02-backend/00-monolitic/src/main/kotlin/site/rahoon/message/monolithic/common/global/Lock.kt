...
class Lock(
    ...
    fun acquireLock(
        key: String,
        ttl: Duration = DEFAULT_TTL,
    ): LockToken? = lockRepository.acquireLock(key, ttl)
    ...
    fun acquireLocks(
        keys: List<String>,
        ttl: Duration = DEFAULT_TTL,
    ): LockToken? = lockRepository.acquireLocks(keys, ttl)

    /** 락을 해제합니다. 토큰에 포함된 정보로 자신의 락만 해제됩니다. */
    fun releaseLock(token: LockToken): Boolean = lockRepository.releaseLock(token)
}
